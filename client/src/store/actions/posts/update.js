...
export function updateOne(id, formData, formDataForLocalUpdate) {
    return async (dispatch, getState) => {
        ...
        try {
                const url = `${getState().config.url.isDev ? (getState().config.url.dev.url+getState().config.url.dev.serverPort) : getState().config.url.url }/api/admin/posts/${id}`
                let headers = {}
            // let headers = {"Content-Type": "application/json; charset=utf-8"}
                ...
                let allNew = getState().posts.all.map((one) => {
                    if (one.id === id) {
