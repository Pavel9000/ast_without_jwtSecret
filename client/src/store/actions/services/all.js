...
export function getAll(_start, _limit) {
    return async (dispatch, getState) => {
        ...
        try {
            // console.log()
                const url = `${getState().config.url.isDev ? (getState().config.url.dev.url+getState().config.url.dev.serverPort) : getState().config.url.url }/api/services?_start=${_start}&_limit=${_limit}`
                // console.log(url)
